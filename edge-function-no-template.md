  ...
  try {
    ...
    if (data.search_results && data.search_results.length > 0) {
      ...
      data.citations.forEach((url, index) => {
        citations.push(normalizePerplexityCitation({ url: url, title: '', snippet: '' }, index + 1, answer))
      })
    }
    ...
    annotations.forEach((annotation, index) => {
      if (annotation.type === 'url_citation') {
        ...
        if (existing) {
          ...
          citationMap.set(annotation.url, { annotation: annotation, positions: [index + 1] })
        }
      }
    })

    const citations: UnifiedCitation[] = Array.from(citationMap.values()).map(
      (item, index) => normalizeOpenAICitation(item.annotation, index + 1, answer, item.positions)
    )
...
function generateSummary(
  ...
  const avgResponseTime = llmResults.length > 0
    ? llmResults.reduce((sum, r) => sum + r.responseTime, 0) / llmResults.length
    : 0
  ...
  for (const key in llmMap) {
    const llmType = llmMap[key as keyof AnalysisResults]
  ...
  return {
    ...
    myDomainCited: myDomainCited,
    myDomainCitationCount: myDomainCitationCount,
    brandMentioned: brandMentioned,
    brandMentionCount: brandMentionCount,
    avgResponseTime: avgResponseTime,
    successfulLLMs: successfulLLMs,
    failedLLMs: failedLLMs,
    citationRateByLLM: citationRateByLLM,
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const analysisId = insertResult.data.id
...
모든 템플릿 리터럴(백틱)을 문자열 연결로 변경했습니다:
- `` `${a}/${b}` `` → `a + '/' + b`
- `` `Bearer ${token}` `` → `'Bearer ' + token`

이제 Supabase Dashboard 에디터에서 파싱 오류가 발생하지 않을 것입니다.
